...
num_workers = 4
if os.name == "nt":
    print("On windows, you may need to set num_workers=0.")
    num_workers = 0

parser = ArgParser(
    description="Assignment 4: super resolution convolutional neural network")
...
train_parser.add_argument("--num-epoch",
                          type=int,
                          default=400,
                          help="number of epochs to train for")
train_parser.add_argument("--save-freq",
                          type=int,
                          default=20,
...
resume_parser.add_argument("--batch-size",
                           type=int,
                           help="training batch size")
resume_parser.add_argument("--num-epoch",
                           type=int,
                           help="number of epochs to train for")
resume_parser.add_argument("--save-freq",
                           type=int,
                           help="save checkpoint every SAVE_FREQ epoches")
resume_parser.add_argument("--lr",
                           type=float,
                           help="learning Rate. default=0.001")
resume_parser.add_argument("--cuda",
                           action="store_true",
                           help="use cuda to train the model?")
...
if opt.mode == "resume":
    ...
    checkpoint = load_checkpoint(opt.checkpoint)
    print("Saved at epoch {} with PSNR={:.4f}".format(checkpoint["epoch"],
                                                      checkpoint["psnr"]))
...
record = {"cur": 0, "best": 0, "epoch": 0, "model": None, "optim": None}
...
def train(epoch, data_loader):
    ...
    print("[Train] Epoch {cur}/{total} complete: Avg. Loss={val:.4f}".format(
        cur=epoch, total=opt.num_epoch, val=loss_metric.average()))
...
def test(epoch, data_loader):
    ...
    with torch.no_grad():
        ...
        print(
            "[Test] Epoch {cur}/{total} complete: Avg. PSNR={val:.4f}".format(
                cur=epoch, total=opt.num_epoch, val=psnr_metric.average()))
